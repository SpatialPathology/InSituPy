...
1. Create (or locate) the target folder in your cloud sync directory, e.g.:
   `<Nextcloud-root>\claude\castalia\projects\insitupy\.log`
2. Open PowerShell **as Administrator** and run:
   ```powershell
   New-Item -ItemType SymbolicLink `
     -Path "<repo-root>\.log" `
     -Target "<cloud-sync-root>\claude\castalia\projects\insitupy\.log"
   ```
...
of your personal cloud sync folder. On the machine where this was first set up, the target is
`C:\Users\ge37voy\Huberwirthwolke\claude\castalia\projects\insitupy\.log`.
