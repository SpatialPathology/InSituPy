<!-- AUTO-GENERATED by tools/generate_skill_reference.py from mcp_server.server.get_public_api() (insitupy v0.12.0b6) - do not edit here; edit the source and re-run the generator. -->

# InSituPy Public API

## Top-level exports (from insitupy import ...)

  InSituData  (type)  — InSituData class for managing and analyzing spatially resolved transcriptomics data.
  InSituExperiment  (type)  — A class to manage and analyze multiple spatially resolved single-cell transcriptomics experiments.
  __author__  (str)
  __email__  (str)
  __version__  (str)
  _core  (module)
  containers  (module)
  datasets  (module)
  experiment  (module)
  experimental  (module)
  im  (module)
  io  (module)
  pl  (module)
  pp  (module)
  tl  (module)
  utils  (module)

## Submodule shorthands

  insitupy.io: I/O — read_xenium, read_visium, read_qupath, etc.
  insitupy.pl (plotting): Plotting — spatial, umap, volcano, overview, etc.
  insitupy.pp (preprocessing): Preprocessing — normalize, filter, pseudobulk, etc.
  insitupy.tl (tools): Analysis — DGE, distance, neighbors, registration, etc.
  insitupy.im (images): Image utilities — read, transform, etc.
  insitupy.utils: Utilities — XeniumPanels, mock data, DGE helpers, etc.
  insitupy.datasets: Example datasets — download and load sample data.
  insitupy.interactive: Interactive napari-based visualization widgets.
  insitupy.spatialdata: SpatialData conversion — to/from SpatialData format.
