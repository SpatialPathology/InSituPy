...
def convert_from_spatialdata(
    sdata: SpatialData,
    nuclei_image_name: str = "morphology_mip",
    ...
    img_name = "nuclei"
    img_data = sdata[nuclei_image_name]

    # extract dimensions for adding boundaries data later
    xmax = img_data.scale0.dims["x"]
    ymax = img_data.scale0.dims["y"]
    ...
    pixel_size = 1 / get_transformation(sdata["cell_circles"]).scale[0].item()
    data.images.add_image(
        image=da_img, name=img_name,
        axes=axes, pixel_size=pixel_size,
        overwrite=False, verbose=verbose
    )
    # data.images.add_image(da_img, img_name, axes=axes, pixel_size=pixel_size,
    #     overwrite=False, verbose=verbose
    #     )
    ...
    cell_bounds = sdata['cell_labels'].scale0['image'].data
    nuc_bounds = sdata['nucleus_labels'].scale0['image'].data

    # if verbose:
    #     print("\tRasterizing boundaries...", flush=True)

    # cell_bounds = _rasterize_boundaries(sdata["cell_boundaries"], xmax=xmax, ymax=ymax)
    # nuc_bounds = _rasterize_boundaries(sdata["nucleus_boundaries"], xmax=xmax, ymax=ymax)
    ...
    # for bound_name in ['cell_boundaries']:
    #     if bound_name in sdata:
    ...
    if "transcripts" in sdata:
        data.transcripts = sdata["transcripts"]
